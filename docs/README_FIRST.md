...
### 1-4. ⚠️ **고도화/업그레이드 시 새로운 시스템 생성 금지** (2025-11-27 추가)

**❌ 심각한 실수 사례 (2025-11-27)**:
...
- 결과: **기존 시스템과 단절**, 다른 컴포넌트와 연동 불가, 유지보수 악몽

**✅ 올바른 접근**:
- **기존 코드를 확장**하여 고품질 결과물 생성
- **기존 데이터 플로우 유지**
- **하위 호환성 보장**

**⚡ 필수 규칙**:
> **고도화/업그레이드 작업 전에 반드시 `docs/UPGRADE_PRINCIPLES.md`를 읽어야 합니다.**
>
> 이 문서를 읽지 않고 작업하면 높은 확률로 잘못된 구현을 하게 됩니다.
...
- ✅ 메인 페이지: `/studio/v3`
- ✅ 메인 엔진 폴더: `frontend/components/canvas-studio/` : 이 엔진은 /studio/v3로 리디렉션 / 항상 메인페이지로 로딩
