...
│  │ └─ PostgreSQL    │        │ └─ MinIO         │         │
│  │                  │        │                  │         │
│  │ ComfyUI (단독)   │        │ 100.123.51.5     │         │
│  │ ├─ Image Gen     │        │ (Tailscale)      │         │
│  │ └─ Video Gen     │        └──────────────────┘         │
│  │                  │                                      │
│  │ 100.120.180.42    │        노트북 (선택)                 │
│  └──────────────────┘        ┌──────────────────┐         │
│                              │ 개발용 클라이언트  │         │
│                              │ ├─ Frontend       │         │
│                              │ └─ Testing        │         │
...
### 📦 설치된 오픈소스 LLM 모델 (Docker Ollama)

| 모델 이름 | 버전 | 크기 | 용도 | 수정 시간 |
|----------|------|------|------|-----------|
| **qwen2.5:7b** | 845dbda0ea48 | 4.7 GB | 빠른 추론 (Brief, Brand, Editor) | 24시간 전 |
| **qwen2.5:14b** | 7cdf5a0187d5 | 9.0 GB | 고품질 생성 (Strategist, Copywriter) | 26시간 전 |
| **mistral-small** | 8039dd90c113 | 14 GB | 복잡한 추론 작업 | 24시간 전 |
| **llama3.2** | a80c4f17acd5 | 2.0 GB | 경량 작업 | 5주 전 |

### ⚠️ A팀이 반드시 알아야 할 사항

1. **테스트 대상 서버**: Mac mini (100.123.51.5)
2. **LLM 서버**: 데스크탑 Docker Ollama (100.120.180.42:11434)
3. **이미지 생성**: 데스크탑 ComfyUI (100.120.180.42:8188)
4. **테스트 환경**: `.env.test` 파일에서 API_BASE_URL 설정 확인 필수

## 목차

1. [시스템 구조](#-시스템-구조-필수-숙지)
2. [A팀 역할 및 책임](#1-a팀-역할-및-책임)
3. [테스트 전략 개요](#2-테스트-전략-개요)
4. [Phase별 테스트 계획](#3-phase별-테스트-계획)
5. [테스트 환경 설정](#4-테스트-환경-설정)
6. [Canvas Studio v3 테스트](#5-canvas-studio-v3-테스트)
7. [Concept Board 테스트](#6-concept-board-테스트)
8. [Generator API 테스트](#7-generator-api-테스트)
9. [성능 테스트](#8-성능-테스트)
10. [버그 리포팅 프로세스](#9-버그-리포팅-프로세스)
11. [DoD (Definition of Done)](#10-dod-definition-of-done)
...
5. **Mac mini 서버 관리 및 동기화** ⚠️ **중요**
   - 매일 작업 시작 전: Mac mini 서버와 로컬 코드 동기화 상태 확인
   - 매일 작업 종료 후: Mac mini 서버로 변경사항 동기화
   - Backend API 변경 시: Mac mini 서버 배포 상태 확인
   - 테스트 환경 설정 파일(.env.test) 관리
...
# 2. Mac mini 서버와 로컬 Git 동기화 확인
git fetch origin
git status

# 3. Backend가 최신 상태인지 확인 (필요 시 B팀에 확인)
# ⚠️ 주의: Mac mini 호출 시 반드시 woosun@100.123.51.5 사용
# ❌ 잘못된 호출: ssh mac-mini 또는 ssh 100.123.51.5
# ✅ 올바른 호출: ssh woosun@100.123.51.5
ssh woosun@100.123.51.5 "cd ~/sparklio_ai_marketing_studio/backend && git log -1 --oneline"

# 4. 로컬과 Mac mini 서버의 커밋 비교
git log origin/master..HEAD --oneline  # 로컬이 앞선 커밋
git log HEAD..origin/master --oneline  # 서버가 앞선 커밋
```

**체크 항목**:
- [ ] Mac mini 서버 응답 확인 (health check)
- [ ] 로컬 코드와 서버 코드 동기화 상태 확인
- [ ] Backend API 변경사항 확인 (B팀 커밋 로그)
- [ ] .env.test 파일이 올바른 설정인지 확인 (API_BASE_URL=http://100.123.51.5:8000)
- [ ] 동기화 문제 발견 시: B팀에 즉시 알림

**⚠️ Mac mini 서버 접속 주의사항**:
- **반드시 `woosun@100.123.51.5` 형식으로 호출해야 합니다**
- `ssh mac-mini` 또는 `ssh 100.123.51.5` 는 작동하지 않습니다
- 올바른 예시: `ssh woosun@100.123.51.5`
- 잘못된 예시: `ssh mac-mini`, `ssh 100.123.51.5`

**⚠️ 동기화 중요성**:
- Mac mini 서버와 코드 동기화가 안 되어 있으면 테스트가 실패할 수 있습니다
- 테스트 실패 시 가장 먼저 서버 동기화 상태를 확인하세요
- **오늘(2025-11-16) 발견된 문제**: Backend API 코드가 Mac mini에 배포되지 않아 189개 테스트 모두 실패

### 11.2 일일 작업 종료 후 체크리스트 ⚠️ **필수**
...
# 2. Mac mini 서버 동기화 상태 확인 (B팀에게 요청)
# B팀이 Backend 변경사항을 Mac mini에 배포했는지 확인
```

**체크 항목**:
- [ ] 모든 변경사항 커밋 및 푸시
- [ ] B팀에게 Mac mini 배포 상태 확인 요청
- [ ] 내일 작업을 위한 서버 준비 완료
