...
Sparklio.ai는 **챗 기반**으로 브랜드 분석 → 브리프 → 산출물(상품상세/브로셔, SNS, 프레젠테이션, 광고/쇼츠) 생성 → **검토 범퍼(초안 승인)** → 통합 에디터/Video Studio 편집 → **자동 발행**까지 단일 경험을 제공하는 **AI 디자인·마케팅 OS**입니다.  
...
|모델|분류|강점|주요 용도|
|---|---|---|---|
|**Gemini 2.5 Flash**|클라우드|속도/가성비|요약, SNS, 실시간 챗|
|**GPT‑4o mini / 5**|클라우드|균형/고품질|초안/고난도 전략|
|**Claude 3.5 Haiku/Sonnet**|클라우드|톤 안정/긴문서|브리프/상품상세|
|**Qwen2‑14B (local)**|로컬|가성비/길이|템플릿/프리젠테이션|
|**Llama3.1‑8B (local)**|로컬|균형|프리젠테이션/요약|
|**Mistral‑7B (local)**|로컬|경량/속도|트렌드 분석|
|**NanoBanana**|클라우드|이미지|썸네일/시각 아이디어|
|**DALL·E**|클라우드|일관성|브랜드 이미지|
|**Sora2**|클라우드|비디오|광고/쇼츠 합성|
|**Pi**|클라우드|대화|가벼운 어시스트|
