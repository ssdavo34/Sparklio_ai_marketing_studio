...
작성일: 2025-12-06
원본: sparklio_ai_marketing_studio/backend/app/services/video_builder.py
"""
...
class EdgeTTSService:
    ...
    async def generate_multiple(self, segments: List[TTSSegment]) -> List[str]:
        """여러 세그먼트 병렬 생성"""
        tasks = [
...
class VideoBuilder:
    ...
    async def _create_scene_videos(
        ...
        for scene in scenes:
            ...
            if not image_path:
                logger.warning(f"[VideoBuilder] No image for scene {scene_num}")
                continue
    ...
    async def _add_bgm(
        ...
        if not os.path.exists(bgm_path):
            logger.warning(f"[VideoBuilder] BGM not found: {bgm_path}")
            return video_path
