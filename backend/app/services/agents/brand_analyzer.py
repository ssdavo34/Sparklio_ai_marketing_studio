    ...
    def _build_task_instructions(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "brand_dna_generation": {
                "instruction": (
                    "당신은 브랜드 전략 전문가입니다. 제공된 문서(documents)의 **실제 내용만을 기반으로** "
                    "Brand DNA Card를 생성하세요.\n\n"

                    "## ⚠️ 절대 금지 사항\n"
                    "- 문서에 없는 정보를 창작하거나 추측하지 마세요\n"
                    "- 일반적인 마케팅 문구나 템플릿을 사용하지 마세요\n"
                    "- 'brand_name' 필드의 이름(예: 'Demo Brand')을 그대로 사용하지 마세요\n"
                    "- 대신 문서 내용에서 실제 브랜드/제품/서비스 이름을 찾아 사용하세요\n\n"

                    "## 분석 방법\n"
                    "1. 각 문서의 title과 extracted_text를 꼼꼼히 읽으세요\n"
                    "2. 문서에서 언급된 구체적인 기능, 특징, 가치를 추출하세요\n"
                    "3. 톤앤매너는 문서의 실제 문체와 표현 방식에서 도출하세요\n"
                    "4. 타겟 오디언스는 문서에서 언급된 사용자/고객 정보를 기반으로 작성하세요\n"
                    "5. Do's/Don'ts는 문서 내용에서 추론 가능한 것만 작성하세요\n"
                ),
                "structure": {
    ...
    async def execute(self, request: AgentRequest) -> AgentResponse:
        ...
        enhanced_payload = {
            **request.payload,
            ...
            # 영문 문서도 한글로 분석하도록 명시 + 문서 기반 분석 강조
            "_context": (
                "## ⚠️ 핵심 지시사항 (반드시 준수)\n\n"

                "1. **문서 기반 분석**: 오직 'documents' 배열에 있는 문서들의 실제 내용만 분석하세요. "
                "각 문서의 'title'과 'extracted_text'를 주의 깊게 읽고, 거기서 정보를 추출하세요.\n\n"

                "2. **브랜드 이름**: 'brand_name' 필드(예: 'Demo Brand')를 무시하세요. "
                "대신 문서 내용에서 실제 브랜드/제품/서비스 이름을 찾아 사용하세요. "
                "예: 문서에 'Polotno SDK'가 언급되면, 'Polotno SDK'를 브랜드 이름으로 사용하세요.\n\n"

                "3. **한국어 출력**: 영어 문서라도 반드시 한국어로 분석 결과를 작성하세요.\n\n"

                "4. **창작 금지**: 문서에 없는 정보를 만들어내지 마세요. "
                "타겟 오디언스, 톤앤매너, Do's/Don'ts 모두 문서 내용에서 도출해야 합니다.\n\n"

                "5. **일반적 문구 금지**: '개발자와 크리에이터', '효율적인 솔루션', '비용 절감' 같은 "
                "일반적인 마케팅 문구 대신, 문서에서 구체적으로 언급된 내용을 사용하세요."
            )
        }
